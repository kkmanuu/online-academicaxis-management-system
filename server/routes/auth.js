...
router.post("/login", async (req, res) => {
  try {
    ...
    if (user.isBlocked) {
      console.log(
        `Login attempt failed: Blocked user ${email} attempted to login`
      );
    ...
    if (role && user.role !== role) {
      console.log(
        `Login attempt failed: Role mismatch for user ${email}. Expected: ${role}, Actual: ${user.role}`
      );
      return res
        .status(400)
        .json({ message: `Invalid credentials for ${role} login` });
    }
