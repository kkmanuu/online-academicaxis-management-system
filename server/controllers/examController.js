...
exports.addQuestions = async (req, res) => {
  try {
    ...
    if (!Array.isArray(questions)) {
      questions = [questions]; // Wrap single question in an array
    }

    // Validate questions
    if (questions.length === 0) {
      console.warn('No questions provided:', req.body);
    ...
    for (const q of questions) {
      ...
      if (!q.question || !Array.isArray(q.options) || q.options.length !== 4 || typeof q.correctAnswer !== 'number' || q.correctAnswer < 0 || q.correctAnswer > 3 || !q.marks) {
        console.warn('Invalid question data:', q);
        return res.status(400).json({ message: 'Invalid question data. Ensure question, 4 options, valid correctAnswer (0-3), and marks are provided.' });
      }

      // Validate unique options
      const uniqueOptions = new Set(q.options.map(opt => opt.trim().toLowerCase()));
      if (uniqueOptions.size !== q.options.length) {
        console.warn('Duplicate options detected:', q.options);
