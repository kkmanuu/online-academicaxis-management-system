...
const BootstrapLoader = () => {
  useEffect(() => {
    ...
    // Shared auth styles — injected once across Login & Register
    if (!document.getElementById("auth-style")) {
      ...
      style.textContent = `
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .auth-orb-1 { width: 520px; height: 520px; background: #4f46e5; top: -140px; left: -100px; }
        ...
        @media (max-width: 991px) {
          .auth-form-panel { max-width: 100%; border-left: none; background: rgba(13,19,33,.8); }
        }
        .auth-form-inner { width: 100%; max-width: 360px; margin: 0 auto; }
        .auth-card {
          background: rgba(255,255,255,.045); border: 1px solid rgba(255,255,255,.09);
          border-radius: 20px; padding: 1.75rem; backdrop-filter: blur(6px);
        }
        ...
        .auth-input-wrap .input-icon {
          ...
          color: #475569; font-size: .95rem; pointer-events: none; transition: color .15s;
        }
        .auth-input-wrap:focus-within .input-icon { color: #818cf8; }
        .auth-input {
          width: 100%; background: rgba(255,255,255,.065);
          border: 1px solid rgba(255,255,255,.11); border-radius: 10px;
          padding: 11px 14px 11px 40px; font-size: .875rem;
          color: #f1f5f9; outline: none; font-family: inherit;
          transition: border-color .15s, background .15s, box-shadow .15s;
        }
        .auth-input::placeholder { color: #4b5563; }
        .auth-input:focus {
          border-color: #4f46e5; background: rgba(79,70,229,.1);
          box-shadow: 0 0 0 3px rgba(79,70,229,.18); color: #f1f5f9;
        }
        ...
        .role-pill.selected { border-color: #4f46e5; background: rgba(79,70,229,.18); color: #a5b4fc; }
        ...
        .auth-btn-primary::after {
          ...
          background: linear-gradient(rgba(255,255,255,.08), transparent); pointer-events: none;
        }
        .auth-btn-primary:hover:not(:disabled) { opacity: .9; transform: translateY(-1px); box-shadow: 0 6px 26px rgba(79,70,229,.45); }
        ...
        .auth-btn-outline {
          ...
          border-radius: 10px; font-size: .875rem; font-weight: 500; color: #94a3b8;
          display: flex; align-items: center; justify-content: center; gap: 8px;
          cursor: pointer; font-family: inherit; text-decoration: none;
          transition: background .15s, border-color .15s, color .15s;
        }
        .auth-btn-outline:hover { background: rgba(255,255,255,.08); border-color: rgba(255,255,255,.22); color: #e2e8f0; }
        ...
        .auth-footer { text-align: center; color: #334155; font-size: .72rem; margin-top: 1.5rem; }
        .pw-bar-wrap { height: 3px; background: rgba(255,255,255,.08); border-radius: 50rem; margin-top: 6px; overflow: hidden; }
        .pw-bar { height: 100%; border-radius: 50rem; transition: width .3s, background .3s; }
        .field-hint { font-size: .72rem; margin: 4px 0 0; display: flex; align-items: center; gap: 5px; }
      `;
