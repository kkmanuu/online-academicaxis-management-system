...
const BootstrapLoader = () => {
  useEffect(() => {
    ...
    // Shared auth styles — only inject once
    if (!document.getElementById("auth-style")) {
      ...
      style.textContent = `
        ...
        .auth-orb-1 { width: 500px; height: 500px; background: #4f46e5; top: -120px; left: -80px; }
        ...
        @keyframes floatOrb { 0% { transform: translateY(0) scale(1); } 100% { transform: translateY(-30px) scale(1.06); } }
        .auth-grid {
          position: fixed; inset: 0; z-index: 0;
        ...
        .auth-form-panel {
          ...
          background: rgba(15,23,42,.6); backdrop-filter: blur(24px);
          border-left: 1px solid rgba(255,255,255,.07);
          overflow-y: auto;
        }
        @media (max-width: 991px) {
          .auth-form-panel { max-width: 100%; background: rgba(15,23,42,.75); border-left: none; }
        }
        ...
        .auth-input {
          ...
          padding: 11px 14px 11px 42px; font-size: .9rem; color: #f1f5f9;
          outline: none; transition: border-color .15s, background .15s, box-shadow .15s;
          font-family: inherit;
        }
        .auth-input::placeholder { color: #64748b; }
        .auth-input:focus { border-color: #4f46e5; background: rgba(79,70,229,.1); box-shadow: 0 0 0 3px rgba(79,70,229,.2); }
        .auth-input:disabled { opacity: .5; cursor: not-allowed; }
        .auth-input-wrap { position: relative; }
        .auth-input-wrap .input-icon {
          position: absolute; left: 14px; top: 50%; transform: translateY(-50%);
          color: #475569; font-size: 1rem; pointer-events: none; transition: color .15s;
        }
        .auth-input-wrap:focus-within .input-icon { color: #818cf8; }
        .auth-label { display: block; font-size: .75rem; font-weight: 700; letter-spacing: .07em; text-transform: uppercase; color: #94a3b8; margin-bottom: 6px; }
        .auth-btn-primary {
          width: 100%; background: linear-gradient(135deg, #4f46e5, #7c3aed);
          border: none; border-radius: 10px; padding: 12px;
          font-size: .95rem; font-weight: 600; color: #fff; cursor: pointer;
          display: flex; align-items: center; justify-content: center; gap: 8px;
          transition: opacity .15s, transform .1s, box-shadow .15s;
          box-shadow: 0 4px 16px rgba(79,70,229,.35); font-family: inherit;
          position: relative; overflow: hidden;
        }
        .auth-btn-primary::after { content: ''; position: absolute; inset: 0; background: linear-gradient(rgba(255,255,255,.1), transparent); }
        ...
        .auth-btn-outline {
          ...
          border: 1px solid rgba(255,255,255,.15); border-radius: 10px; padding: 12px;
          font-size: .9rem; font-weight: 500; color: #cbd5e1; cursor: pointer;
          display: flex; align-items: center; justify-content: center; gap: 8px;
        ...
        .auth-error {
          ...
          font-size: .85rem; color: #fca5a5; margin-bottom: 1.25rem;
        }
        .role-pill {
        ...
        .role-pill.selected { border-color: #4f46e5; background: rgba(79,70,229,.2); color: #818cf8; }
        ...
        .pw-strength-bar {
          height: 3px; border-radius: 50rem;
        ...
        <div className="auth-brand-panel">
          <div style={{ width: '100%', maxWidth: 440 }}>
            ...
            <p style={{ color: '#94a3b8', fontSize: '1.05rem', lineHeight: 1.7, marginBottom: '2.5rem' }}>
              Create your free account and start your academic journey today. Connect with expert teachers and access world-class resources.
            </p>

            {/* Steps */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {[
                { n: '01', icon: 'bi-person-plus-fill', color: '#818cf8', label: 'Create your account', sub: 'Fill in your details below' },
                { n: '02', icon: 'bi-journals', color: '#34d399', label: 'Enrol in courses', sub: 'Browse and join courses that interest you' },
                { n: '03', icon: 'bi-trophy-fill', color: '#fbbf24', label: 'Achieve your goals', sub: 'Track progress and earn certificates' },
              ].map((step, i) => (
                <div key={step.n} style={{ display: 'flex', gap: 16, paddingBottom: i < 2 ? '1.5rem' : 0 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(255,255,255,.08)', border: '1px solid rgba(255,255,255,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <i className={`bi ${step.icon}`} style={{ fontSize: '1.15rem', color: step.color }}></i>
                    </div>
                    {i < 2 && <div style={{ width: 1, flex: 1, background: 'rgba(255,255,255,.08)', margin: '6px 0' }}></div>}
                  </div>
                  <div style={{ paddingTop: 8 }}>
                    <p style={{ margin: 0, fontWeight: 600, color: '#e2e8f0', fontSize: '.9rem' }}>{step.label}</p>
                    <p style={{ margin: 0, color: '#64748b', fontSize: '.78rem' }}>{step.sub}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="auth-form-panel">
          <div className="auth-form-inner">
            ...
            <div className="d-lg-none text-center mb-4">
              ...
              <p style={{ color: '#64748b', fontSize: '.85rem', margin: '.25rem 0 0' }}>Create your free account</p>
            </div>
            ...
            <div className="auth-card">
              ...
              <form onSubmit={handleSubmit}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {roleConfig.map((r) => (
                      <button key={r.value} type="button"
                        ...
                        onClick={() => handleRoleSelect(r.value)} disabled={loading}>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <div className="auth-input-wrap">
                    ...
                    <input className="auth-input" type={showPassword ? 'text' : 'password'}
                      ...
                      onChange={handleChange} disabled={loading} style={{ paddingRight: 44 }} />
                  ...
                  {formData.password && (
                    <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ flex: 1, height: 3, background: 'rgba(255,255,255,.08)', borderRadius: '50rem', overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: strength.pct, background: strength.color, borderRadius: '50rem', transition: 'width .3s, background .3s' }}></div>
                      </div>
                      <span style={{ fontSize: '.72rem', fontWeight: 600, color: strength.color, whiteSpace: 'nowrap' }}>{strength.label}</span>
                    </div>
                  )}
                </div>
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  ...
                  <div className="auth-input-wrap">
                    ...
                    <input className="auth-input" type={showConfirm ? 'text' : 'password'}
                      ...
                      style={{ paddingRight: 44, borderColor: passwordsMismatch ? '#ef4444' : passwordsMatch ? '#22c55e' : undefined }} />
                    <button type="button" onClick={() => setShowConfirm(!showConfirm)}
                      style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#475569', cursor: 'pointer', padding: 0 }}>
                      <i className={`bi ${showConfirm ? 'bi-eye-slash' : 'bi-eye'}`} style={{ fontSize: '1rem' }}></i>
                    </button>
                  </div>
                  {passwordsMismatch && (
                    <p style={{ margin: '4px 0 0', fontSize: '.75rem', color: '#f87171' }}>
                      <i className="bi bi-x-circle me-1"></i>Passwords do not match
                    </p>
                  )}
                  {passwordsMatch && (
                    <p style={{ margin: '4px 0 0', fontSize: '.75rem', color: '#4ade80' }}>
                ...
                <button type="submit" className="auth-btn-primary" disabled={loading} style={{ marginBottom: 12 }}>
                  {loading
                    ? <><div className="spin-anim"></div> Creating account…</>
