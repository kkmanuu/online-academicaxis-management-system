      ...
      const validExams = Array.isArray(response.data)
        ? response.data.filter((exam) => {
            ...
            const hasValidDates =
              exam.startTime &&
              exam.endTime &&
              !isNaN(new Date(exam.startTime)) &&
              !isNaN(new Date(exam.endTime));
            if (!isValidId) {
  ...
  return (
    ...
    >
      ...
      {exams.length === 0 ? (
        ...
        >
          <Typography variant="h6" sx={{ color: "#455a64", mb: 2 }}>
            ...
            <TableBody>
              {exams.map((exam, index) => (
                ...
                >
                  ...
                  <TableCell sx={{ color: "#455a64" }}>
                    {exam.startTime
                      ? format(parseISO(exam.startTime), "PPpp")
                      : "N/A"}
                  </TableCell>
                  <TableCell sx={{ color: "#455a64" }}>
                    {exam.endTime
                      ? format(parseISO(exam.endTime), "PPpp")
                      : "N/A"}
                  </TableCell>
